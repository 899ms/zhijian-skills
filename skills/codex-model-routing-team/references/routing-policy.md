...
2. [Surface 选择策略](surface-selection-policy.md)：默认原生 V2 Luna leaf Worker；worktree、侧栏、跨任务恢复和耐久监督使用 App Thread。
...
| --- | --- | --- | --- | --- |
| Luna Native | `native_subagent` | `gpt-5.6-luna` XHigh | Standard / live-gated Fast | 默认 leaf Worker |
| Luna Native Max | `native_subagent` | `gpt-5.6-luna` Max | Standard / live-gated Fast | 高难、高风险、高歧义单元 |
| Sol Native | `native_subagent` | `gpt-5.6-sol` High / XHigh / Max | Standard / 显式 Fast | 关键实现、审查、裁决与 Luna fallback |
| Luna App | `app_thread` | `gpt-5.6-luna` XHigh / Max | Standard / live-gated Fast | worktree、侧栏、跨任务恢复、耐久监督 |
| Sol App | `app_thread` | `gpt-5.6-sol` High / XHigh / Max | Standard / 显式 Fast | App 路径中的架构、调试与审查 |
...
Ultra 永久禁止。Luna 自动路由只允许 XHigh/Max；Sol 只允许 High/XHigh/Max。`thinking` 在原生映射为 `reasoning_effort`，在 App Thread 映射为 `thinking`。
...
| --- | --- | --- |
| `DEFAULT_GENERAL` | high | Luna Native XHigh → Sol Native High |
| `HIGH_RISK_GENERAL` | xhigh | Luna Native Max → Sol Native XHigh |
| `DURABLE_WORKSPACE` | high | Luna App XHigh/Max → Sol App High |
| `FAST_MECHANICAL` | xhigh | Luna Native Fast/Standard → Sol Native High Standard |
...
首项被静态门排除时从下一项开始，不得伪称运行时失败。fallback 必须满足最低 thinking、Provider 与数据边界。Terra 只能作为显式请求的首项，不能静默 fallback。
