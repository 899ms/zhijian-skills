...
存在以下硬条件时优先使用 `app_thread`：涉及外部/生产/账户/成本审批；需要跨任务恢复、worktree、独立任务历史或严格 Thread 审计；用户明确要求 App Thread、项目管理或持续执行。

预计超过 30 分钟或有 4 个以上正式交付物只是耐久信号，不单独触发。只有它们同时伴随持久化、恢复或独立审计需求时才升级；短时、文件所有权互斥、回到父任务集成的多文件工作仍可使用 `native_subagent`。
