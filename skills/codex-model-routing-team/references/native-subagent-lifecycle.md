...
- live `spawn_agent` schema 必须暴露 `model` 与 `reasoning_effort`，并接受 RoutePlan 的精确组合；禁止静默继承父模型或默认 reasoning 来冒充路由成功。
...
2. 返回正式 `agent_id` 后记录 `platform_accepted_model`；这不等于观测到真实运行模型。
3. 只在主流程需要结果时有界等待。完整输出质量不足时，同一正式 Agent 最多追问一次。
4. 主 Agent 验证输出并设置 `adopted=true`。
5. 已完成且已采纳的 Agent 调用 close，成功后进入 `CLOSED` 并释放并发位。

`Unknown model`、reasoning 不支持或白名单为空属于精确组合失败。若 RoutePlan 预声明下一候选，可以沿链继续；否则主 Agent 接管。禁止在失败后临时换 Surface、模型、强度或 Provider。

## 审计

记录遵守 `native-audit-schema.json`。区分 `requested_model`、`platform_accepted_model` 与 `observed_runtime_model`；没有可信运行时回显时，`observed_runtime_model` 必须保持 `unknown`。完整结果来自正式 Agent 的 completed message，不能用 task 文本或创建回执代替。
