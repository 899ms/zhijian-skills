...
- live `spawn_agent` schema 必须暴露 `model` 与 `reasoning_effort`，并接受 RoutePlan 的精确组合；Fast 候选还必须暴露并接受 `service_tier=priority`。禁止静默继承父模型、默认 reasoning 或默认速度来冒充路由成功。
...
记录遵守 `native-audit-schema.json`。模型使用 `requested_model/platform_accepted_model/observed_runtime_model`，速度使用 `requested_speed/platform_accepted_speed/observed_runtime_speed`；没有可信运行时回显时，对应 observed 字段必须保持 `unknown`。完整结果来自正式 Agent 的 completed message，不能用 task 文本或创建回执代替。
