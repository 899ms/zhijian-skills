...
- `gpt-5.6-luna` 可以被 V2 父 Agent 作为原生 leaf Worker 创建；它自身仍不获得协作工具，不能继续派生 Worker。
...
- Rollback boundary：如果当前 live schema 不再接受 Native Luna，只熔断该精确原生 tuple，并沿预声明 App Thread 或 Sol fallback 前进；不得把 registry 改回“所有 Luna 永久 App-only”，也不得静默继承父模型。
