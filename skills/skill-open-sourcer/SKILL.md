...
python3 "$SKILL_OPEN_SOURCER_DIR/scripts/release_portfolio.py" plan \
  --repo <zhijian-skills> --skill <name> --dry-run \
...
9. Verify local `npx skills` discovery and one isolated copy install from the canonical repository. Inspect the installed tree; listing success alone is insufficient. Use top-level help only: `npx --no-install skills --help`. Never run `npx skills add <source> --help`; `skills` CLI 1.5.x may perform a real installation because `<source>` is already a valid add request.
10. Commit and push canonical `main` only when publishing is authorized. Never call `gh repo create` or publish to `<owner>/<skill-name>`.
11. Verify the remote Portfolio listing and isolated install, then create the canonical Tag and launch copy.
